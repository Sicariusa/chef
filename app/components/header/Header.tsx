...
export function Header({ hideSidebarIcon = false }: { hideSidebarIcon?: boolean }) {
  ...
  return (
    <header className="relative z-50 flex h-[var(--header-height)] items-center overflow-x-auto overflow-y-hidden border-b border-white/10 bg-gradient-to-b from-black/40 via-black/30 to-transparent backdrop-blur-xl p-5 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)]">
      ...
      <div className="relative z-40 flex cursor-pointer items-center gap-4 text-content-primary">
        {showSidebarIcon && (
          <HamburgerMenuIcon
            className="shrink-0 text-slate-200 hover:text-white transition-colors duration-200"
            data-hamburger-menu
            onClick={(e) => {
              e.stopPropagation();
        ...
        <a
          ...
          className="group relative hidden cursor-pointer select-none items-center gap-2 whitespace-nowrap rounded-lg border border-white/10 bg-gradient-to-br from-slate-900/80 via-slate-800/60 to-slate-900/80 backdrop-blur-md px-3 py-1.5 text-sm font-medium text-slate-200 transition-all duration-300 hover:border-teal-400/30 hover:bg-gradient-to-br hover:from-slate-800/90 hover:via-slate-700/70 hover:to-slate-800/90 hover:text-white hover:shadow-[0_4px_20px_rgba(94,234,212,0.15)] focus-visible:border-teal-400/50 focus-visible:outline-none sm:flex"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" className="transition-transform duration-300 group-hover:scale-110">
            <path
          ...
          <span className="font-display font-medium tracking-wide">Star on GitHub</span>
        </a>
      </div>
      <>
        {chat.started && (
          <span className="flex-1 truncate px-4 text-center font-display text-slate-200/90">
